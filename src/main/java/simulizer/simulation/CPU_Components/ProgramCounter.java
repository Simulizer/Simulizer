...
/**this class represents the Program Counter, a special register in memory
 * it does almost the same as a normal register, but has different bus connections
 * it also has the ability to increment itself for the next address it should store
...
public class ProgramCounter extends Observable
{
	...
	/**this method sets the value in the PC
	 * this method should be used cautiously, i.e only used on the use
	 * of jump instructions, new programs etc.
	...
	/**this method will retrieve whatever data was sent on the IR Bus
	 * this bus connection might actually be redundant?
	 */
